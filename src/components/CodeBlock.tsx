...
function colorLine(line: string) {
  ...
  const parts = line.split(
    /(\b(?:import|from|const|let|function|return|export|type|interface|if|else|new|class|@theme|@import|@layer|@apply)\b|<\/?[A-Za-z][\w.-]*|'[^']*'|"[^"]*"|`[^`]*`)/g,
  );
  return parts.map((p, i) => {
    if (!p) return null;
    if (
      /^(import|from|const|let|function|return|export|type|interface|if|else|new|class|@theme|@import|@layer|@apply)$/.test(
        p,
      )
    ) {
      return (
